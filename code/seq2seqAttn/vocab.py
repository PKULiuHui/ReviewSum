...
class Vocab:
    def __init__(self, args, embed=None):
        ...
        self.UNK_TOKEN = '<UNK>'
        self.next_idx = 4
        self.word2id = {self.PAD_TOKEN: self.PAD_IDX, self.SOS_TOKEN: self.SOS_IDX, self.EOS_TOKEN: self.EOS_IDX, self.UNK_TOKEN: self.UNK_IDX}
        self.id2word = {v: k for k, v in self.word2id.items()}
        self.word2cnt = {self.PAD_TOKEN: 10000, self.SOS_TOKEN: 10000, self.EOS_TOKEN: 10000, self.UNK_TOKEN: 10000}  # 给这三个标记较大的值防止后面被删去
        for i in range(4):
    ...
    def make_tensors(self, batch):
        ...
        for summary in trg_text:
            summary = [self.SOS_TOKEN] + summary.split() + [self.EOS_TOKEN]
