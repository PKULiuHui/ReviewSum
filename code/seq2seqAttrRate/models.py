...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, src, trg, src_, trg_, user, product, rating, vocab_size, test=False):
        ...
        if self.args.rating_method == 'max':
            rat_input = []
        ...
        elif self.args.rating_method == 'avg':
            rat_input = []
            for i in range(batch_size):
        ...
        # rating_embed = self.rating_embed(rating) if not test else self.rating_embed(pre_rating)
