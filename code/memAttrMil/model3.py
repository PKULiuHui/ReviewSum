...
# 使用word level MIL预测情感，同时使用预测结果辅助摘要生成
...
class Model(nn.Module):
    ...
    def __init__(self, args, embed=None):
        ...
        self.senti_hidden = nn.Linear(3 * args.hidden_size, args.hidden_size)
        self.attention_senti = Attention(args.hidden_size, key_size=args.hidden_size, query_size=2 * args.hidden_size)
        # context gate
        self.context_gate = nn.Linear(6 * args.hidden_size + args.embed_dim + args.attr_dim, 4)
    ...
    def decode_step(self, src, prev_embed, encoder_hidden, senti_hidden, src_mask, proj_key, proj_key_senti,
                    encoder_attr, proj_key_attr, hidden, context_hidden, vocab_size, mem_out, highway, review_rating):
        ...
        context_senti, _ = self.attention_senti(query=torch.cat([query, review_rating], dim=-1),
                                                proj_key=proj_key_senti, value=senti_hidden, mask=src_mask)
        ...
        context_g = F.softmax(self.context_gate(
            torch.cat([query, prev_embed, context_text, context_attr, context_senti, mem_out], dim=-1)), dim=-1)
        context = torch.cat([text_context, attr_context, mem_context, senti_context], dim=1)
    ...
    def forward(self, src, trg, src_, trg_, user, product, rating, vocab_size, u_review, u_sum, p_review, p_sum,
        ...
        senti_hidden = self.senti_hidden(torch.cat([encoder_hidden, word_rating], dim=-1))
        ...
        highway = self.highway_fusion(
            torch.cat([user_embed.unsqueeze(1), product_embed.unsqueeze(1), mem_out, review_rating],
                      dim=-1)).contiguous()
