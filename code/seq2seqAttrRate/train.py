...
# Use seq2seq + Luong attention + copy mechanism + attributes + rating prediction to generate
...
parser = argparse.ArgumentParser(description='seq2seqAttrRate')
# path info
...
parser.add_argument('-attr_dim', type=int, default=50)
...
parser.add_argument('-rating_method', type=str, default='avg')
parser.add_argument('-review_max_len', type=int, default=1000)
...
def evaluate(net, criterion, vocab, data_iter, train_next=True):
    ...
    sums = []
    ratings, pre_ratings = [], []
    loss, r1, r2, rl, acc = .0, .0, .0, .0, .0
    rouge = RougeCalculator(stopwords=False, lang="en")
    for batch in tqdm(data_iter):
        src, trg, src_, trg_, src_user, src_product, src_rating, src_text, trg_text = vocab.read_batch(batch)
        sum_out, rat_out = net(src, trg, src_, trg_, src_user, src_product, src_rating, vocab.word_num, test=True)
        sum_out_1, rat_out_1 = net(src, trg, src_, trg_, src_user, src_product, src_rating, vocab.word_num, test=False)
...
def train():
    ...
    for epoch in range(args.begin_epoch, args.epochs + 1):
        ...
        for i, batch in enumerate(train_iter):
            src, trg, src_, trg_, src_user, src_product, src_rating, _1, _2 = vocab.read_batch(batch)
            ...
            if cnt % args.valid_every == 0:
                ...
                cur_loss, r1, r2, rl, acc = evaluate(net, criterion, vocab, val_iter, True)
                save_path = args.save_path + 'valid_%d_%.4f_%.4f_%.4f_%.4f_%.4f' % (
                    cnt / args.valid_every, cur_loss, r1, r2, rl, acc)
                net.save(save_path)
                print('Epoch: %2d Cur_Val_Loss: %f Rouge-1: %f Rouge-2: %f Rouge-l: %f Acc: %f' %
                      (epoch, cur_loss, r1, r2, rl, acc))
    return
...
def test():
    ...
    test_iter = DataLoader(dataset=test_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=my_collate)
    ...
    print('Begin testing...')
    loss, r1, r2, rl, acc = evaluate(net, criterion, vocab, test_iter, False)
    print('Loss: %f Rouge-1: %f Rouge-2: %f Rouge-l: %f Acc: %f' % (loss, r1, r2, rl, acc))
