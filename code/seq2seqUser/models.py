...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, args, embed):
        ...
        # Decoder
        self.decoder_rnn = nn.GRU(args.embed_dim + args.user_dim, args.hidden_size, args.num_layers,
        ...
        self.pre_output_layer = nn.Linear(args.hidden_size + args.embed_dim + args.user_dim, args.hidden_size)
        # Generator
        self.generator = nn.Linear(args.hidden_size, args.embed_num)

    def decode_step(self, decode_input, hidden):
        """Perform a single decoder step (1 word)"""
        output, hidden = self.decoder_rnn(decode_input, hidden)
        pre_output = torch.cat([decode_input, output], dim=2)
        pre_output = self.dropout_layer(pre_output)
        pre_output = self.pre_output_layer(pre_output)

        return output, hidden, pre_output

    def forward(self, src, trg, src_lengths, trg_lengths, src_user, test=False):
        # embed user
        ...
        for i in range(max_len):
            ...
            encode_input = torch.cat([prev_embed, user_embed], dim=2)
            output, hidden, pre_output = self.decode_step(encode_input, hidden)
