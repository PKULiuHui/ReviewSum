...
class EncoderDecoder(nn.Module):
    ...
    def decode_step(self, src, prev_embed, encoder_hidden, src_mask, proj_key, encoder_attr, proj_key_attr, hidden,
        ...
        text_context = F.leaky_relu(self.text_context(context_text))
        attr_context = F.leaky_relu(self.attr_context(context_attr))
    ...
    def forward(self, src, trg, src_, trg_, user, product, vocab_size, src_mask, src_lengths, trg_lengths, test=False):
        ...
        mix_embed = F.leaky_relu(self.user_product_mix(torch.cat([user_embed, product_embed], dim=-1)))  # [B, A]
        ...
        encoder_g = F.sigmoid(self.encoder_gate(torch.cat([encoder_final[-1], user_embed, product_embed, mix_embed], dim=-1)))
        encoder_final = encoder_g * text_final.contiguous().view(len(src), -1) + (1 - encoder_g) * attr_final.contiguous().view(len(src), -1)
