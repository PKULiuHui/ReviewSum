...
class MemAttrCov(nn.Module):

    def __init__(self, args, embed=None):
        super(MemAttrCov, self).__init__()
        self.name = 'seq2seqCopyAttr + split memory + coverage mechanism'
        self.args = args
        self.coverage = False
    ...
    def decode_step(self, src, prev_embed, user, product, encoder_hidden, src_mask, proj_key, encoder_attr,
                    proj_key_attr, hidden, context_hidden, mem_out, vocab_size, c):
        ...
        if self.coverage:
            ...
            context_text, attn_probs = self.attention(query=query, proj_key=proj_key, value=encoder_hidden,
                                                      mask=src_mask, proj_cov=proj_cov)
        else:
            context_text, attn_probs = self.attention(query=query, proj_key=proj_key, value=encoder_hidden,
                                                      mask=src_mask)
        c = c + attn_probs
    ...
    def forward(self, src, trg, src_, trg_, user, product, vocab_size, u_review, u_sum, p_review, p_sum, test=False):
        ...
        for i in range(max_len):
            ...
            hidden, context_hidden, word_prob, c, a = self.decode_step(src, prev_embed, user_embed, product_embed,
