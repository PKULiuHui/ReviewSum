...
class Vocab:
    ...
    def make_tensors(self, batch):
        ...
        src, trg, src_user = torch.LongTensor(src), torch.LongTensor(trg), torch.LongTensor(src_user)
        if self.args.use_cuda:
            src, trg, src_user = src.cuda(), trg.cuda(), src_user.cuda()

        return src, trg, src_lens, trg_lens, src_user, src_text, trg_text
