...
class MemAttrRate(nn.Module):
    ...
    def __init__(self, args, embed=None):
        ...
        self.name = 'Memory + Attributes + Gate Fusion + memory rating prediction'
        self.args = args
        ...
        self.rat_predict_1 = nn.Linear(4 * args.hidden_size, args.hidden_size)
    ...
    def forward(self, src, trg, src_, trg_, user, product, rating, vocab_size, u_review, u_sum, p_review, p_sum, test=False):
        ...
        for i in range(batch_size):
            ...
            encoder_avg.append(cur_input / src_lens[i])
        encoder_avg = torch.cat(encoder_avg, dim=-1).view(batch_size, -1)
        rat_input = torch.cat([encoder_avg, mem_out.squeeze(1)], dim=-1)
