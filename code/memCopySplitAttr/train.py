...
from models import MemCopySplitAttr
from sumeval.metrics.rouge import RougeCalculator

parser = argparse.ArgumentParser(description='memCopySplitAttr')
# path info
...
parser.add_argument('-mem_layers', type=int, default=1)
...
def evaluate(net, criterion, vocab, data_iter, train_data, train_next=True):
    ...
    for batch in tqdm(data_iter):
        ...
        sum_out, _1, _2 = net(src, trg, src_embed, trg_embed, src_user, src_product, vocab.word_num, u_review, u_sum,
...
def train():
    ...
    net = MemCopySplitAttr(args, embed)
    if args.load_model is not None:
        print('Loading model...')
        checkpoint = torch.load(args.save_path + args.load_model)
        net = MemCopySplitAttr(checkpoint['args'], embed)
    ...
    for epoch in range(args.begin_epoch, args.epochs + 1):
        for i, batch in enumerate(train_iter):
            ...
            src, trg, src_, trg_, src_user, src_product, u_review, u_sum, p_review, p_sum, _1, _2 = vocab.make_tensors(
                batch, train_data)
            sum_output, g1, g2 = net(src, trg, src_, trg_, src_user, src_product, vocab.word_num, u_review, u_sum,
                                     p_review, p_sum)
