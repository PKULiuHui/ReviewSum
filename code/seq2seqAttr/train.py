...
# Use seq2seq + Luong attention + copy mechanism + attributes to generate amazon review summaries.
# 模型与seq2seqCopyAttr大体上一致，细节上向memAttr attr部分看齐
...
parser = argparse.ArgumentParser(description='seq2seqAttr')
# path info
...
def evaluate(net, criterion, vocab, data_iter, train_next=True):
    ...
    for batch in tqdm(data_iter):
        src, trg, src_, trg_, src_user, src_product, src_text, trg_text = vocab.read_batch(batch)
        sum_out = net(src, trg, src_, trg_, src_user, src_product, vocab.word_num, test=True)
        sum_out_1 = net(src, trg, src_, trg_, src_user, src_product, vocab.word_num, test=False)
...
def train():
    ...
    for epoch in range(args.begin_epoch, args.epochs + 1):
        ...
        for i, batch in enumerate(train_iter):
            src, trg, src_, trg_, src_user, src_product, _1, _2 = vocab.read_batch(batch)
            sum_output = net(src, trg, src_, trg_, src_user, src_product, vocab.word_num)
